...
export const calculateSwapImpact = (
  ...
  fromRole: string,
  toRole: string
): { fromPlayer: string; toPlayer: string; overall: string } => {
  ...
  const fromScoreNew = getTypeScore(toPlayer.profile.type, roleTemplates[toRole] || roleTemplates['侦探']);
  const fromScoreOld = getTypeScore(fromPlayer.profile.type, roleTemplates[fromRole] || roleTemplates['侦探']);
  const toScoreNew = getTypeScore(fromPlayer.profile.type, roleTemplates[fromRole] || roleTemplates['侦探']);
  const toScoreOld = getTypeScore(toPlayer.profile.type, roleTemplates[toRole] || roleTemplates['侦探']);
  ...
  const generateImpactText = (diff: number, isRequester: boolean): string => {
    if (diff > 5) return isRequester ? '对你来说更适配！+10分体验感' : '对方会更适合这个角色';
    if (diff > 0) return isRequester ? '稍微更适合你' : '对方体验基本不变';
    if (diff > -5) return isRequester ? '体验可能略有下降' : '对方可能需要适应一下';
    return isRequester ? '这个角色可能不太适合你...' : '对方可能会玩得有点别扭';
  };
  ...
  else if (overallDiff > -8) overall = '可能会降低整体体验，建议三思';
